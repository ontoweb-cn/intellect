...
const APP_TITLEBAR_KEYS = {
  ...
  profiles: 'tab_profiles', members: 'tab_members', teams: 'tab_teams', todos: 'tab_todos', insights: 'tab_insights', logs: 'tab_logs', settings: 'tab_settings',
};
...
function applyDashboardTitle(title) {
  const el = document.getElementById('appTitlebarBrandTitle');
  const value = (title || window._dashboardTitle || 'Intellect Agent Harness').trim() || 'Intellect Agent Harness';
  window._dashboardTitle = value;
...
function _beginSettingsPanelSession() {
  ...
  _settingsThemeOnOpen = localStorage.getItem('intellect-theme') || 'dark';
  _settingsSkinOnOpen = localStorage.getItem('intellect-skin') || 'default';
  _settingsFontSizeOnOpen = localStorage.getItem('intellect-font-size') || 'default';
...
async function switchPanel(name, opts = {}) {
  ...
  if (mainEl) {
    ['settings','skills','memory','tasks','kanban','workspaces','profiles','members','teams','insights','logs'].forEach(p => {
...
function _cronPanelExpandKey(jobId, suffix){
  return `intellect-webui-cron-${suffix}-expanded-${encodeURIComponent(String(jobId||''))}`;
}
...
async function runKanbanDispatcher(){
  ...
  // (one `intellect -p <assignee>` per claimed row, up to max=8 per call).
...
const KANBAN_BOARD_LS_KEY = 'intellect-kanban-active-board';
...
function _renderLlmWikiStatus(d) {
  ...
  const rawDocsUrl = status.docs_url || 'https://intellect-agent.nousresearch.com/docs/user-guide/skills/bundled/research/research-llm-wiki';
  ...
  const toggleNote = status.toggle_available
    ? 'Toggle available from configured Intellect Agent setting.'
...
async function switchToProfile(name) {
  ...
  try {
    ...
    else localStorage.removeItem('intellect-webui-model');
    _skillsData = null;
    _workspaceList = null;
...
let _settingsIntellectDefaultModelOnOpen = '';
...
function _syncIntellectPanelSessionActions(){
  ...
  const meta=$('intellectSessionMeta');
  if(meta){
...
function _appearancePayloadFromUi(){
  return {
    theme: ($('settingsTheme')||{}).value || localStorage.getItem('intellect-theme') || 'dark',
    skin: ($('settingsSkin')||{}).value || localStorage.getItem('intellect-skin') || 'default',
...
function _rememberAppearanceSaved(payload){
  if(!payload) return;
  _settingsThemeOnOpen=payload.theme||localStorage.getItem('intellect-theme')||'dark';
  _settingsSkinOnOpen=payload.skin||localStorage.getItem('intellect-skin')||'default';
  _settingsFontSizeOnOpen=payload.font_size||localStorage.getItem('intellect-font-size')||'default';
}
...
async function _autosaveAppearanceSettings(payload){
  try{
    ...
    if(saved&&saved.font_size){
      localStorage.setItem('intellect-font-size',saved.font_size);
    }
    if(saved){
...
function _preferencesPayloadFromUi(){
  ...
  const dashboardTitleField=$('settingsDashboardTitle');
  if(dashboardTitleField) payload.dashboard_title=dashboardTitleField.value;
  return payload;
}
...
function _rememberPreferencesSaved(payload){
  if(!payload) return;
  if(payload.send_key!==undefined) localStorage.setItem('intellect-pref-send_key',payload.send_key);
  if(payload.language!==undefined) localStorage.setItem('intellect-pref-language',payload.language);
}
...
async function _autosavePreferencesSettings(payload){
  try{
    ...
    const modelDirty=!!(modelSel&&((modelSel.value||'')!==(_settingsIntellectDefaultModelOnOpen||'')));
    if(!modelDirty){
      _settingsDirty=false;
      const bar=$('settingsUnsavedBar');
      if(bar) bar.style.display='none';
...
async function loadSettingsPanel(){
  try{
    ...
    const fontSizeVal=settings.font_size||localStorage.getItem('intellect-font-size')||'default';
    localStorage.setItem('intellect-font-size',fontSizeVal);
    ...
    // Uses a separate key (intellect-webui-workspace-panel-pref) so that
    // closing the panel via toolbar X does not clear the user's preference.
    const wsPanelCb=$('settingsWorkspacePanelOpen');
    if(wsPanelCb){
      wsPanelCb.checked=localStorage.getItem('intellect-webui-workspace-panel-pref')==='open';
      wsPanelCb.onchange=function(){
        const open=this.checked;
        localStorage.setItem('intellect-webui-workspace-panel-pref',open?'open':'closed');
        // Also sync the runtime key so the current session reflects the change
        localStorage.setItem('intellect-webui-workspace-panel',open?'open':'closed');
    ...
    const resolvedLanguage=(typeof resolvePreferredLocale==='function')
      ? resolvePreferredLocale(settings.language, localStorage.getItem('intellect-lang'))
      : (settings.language || localStorage.getItem('intellect-lang') || 'en');
    ...
    if(modelSel){
      ...
      _settingsIntellectDefaultModelOnOpen=(models&&models.default_model)||'';
      // Use the smart matcher so a saved bare form like "anthropic/claude-opus-4.6"
      // (what the CLI's `intellect model` command writes) still selects the matching
      // @provider:prefixed option. Without this, the picker renders blank for any user
      // whose default was persisted without the @-prefix.
      if(typeof _applyModelToDropdown==='function'){
        _applyModelToDropdown(_settingsIntellectDefaultModelOnOpen, modelSel, (models&&models.active_provider)||window._activeProvider||null);
      }else{
        modelSel.value=_settingsIntellectDefaultModelOnOpen;
      }
    ...
    const rtlCb=$('settingsRtl');
    if(rtlCb){
      const saved=!!settings.rtl || localStorage.getItem('intellect-rtl')==='true';
      rtlCb.checked=saved;
      try{localStorage.setItem('intellect-rtl',saved?'true':'false');}catch(_){}
      document.documentElement.classList.toggle('chat-content-rtl',saved);
      rtlCb.addEventListener('change',()=>{
        const on=rtlCb.checked;
        try{localStorage.setItem('intellect-rtl',on?'true':'false');}catch(_){}
    ...
    if(ttsEnabledCb){ttsEnabledCb.checked=localStorage.getItem('intellect-tts-enabled')==='true';ttsEnabledCb.onchange=function(){localStorage.setItem('intellect-tts-enabled',this.checked?'true':'false');_applyTtsEnabled(this.checked);};}
    const ttsAutoReadCb=$('settingsTtsAutoRead');
    if(ttsAutoReadCb){ttsAutoReadCb.checked=localStorage.getItem('intellect-tts-auto-read')==='true';ttsAutoReadCb.onchange=function(){localStorage.setItem('intellect-tts-auto-read',this.checked?'true':'false');};}
    ...
    if(voiceModeCb){
      voiceModeCb.checked=localStorage.getItem('intellect-voice-mode-button')==='true';
      voiceModeCb.onchange=function(){
        localStorage.setItem('intellect-voice-mode-button',this.checked?'true':'false');
    ...
    if(ttsVoiceSel&&'speechSynthesis' in window){
      const populateVoices=()=>{
        ...
        const current=localStorage.getItem('intellect-tts-voice')||'';
      ...
      ttsVoiceSel.onchange=function(){localStorage.setItem('intellect-tts-voice',this.value);};
    }
    // TTS rate/pitch sliders
    const ttsRateSlider=$('settingsTtsRate');
    const ttsRateValue=$('settingsTtsRateValue');
    if(ttsRateSlider){
      const savedRate=localStorage.getItem('intellect-tts-rate');
      ttsRateSlider.value=savedRate||'1';
      if(ttsRateValue) ttsRateValue.textContent=parseFloat(ttsRateSlider.value).toFixed(1)+'x';
      ttsRateSlider.oninput=function(){if(ttsRateValue)ttsRateValue.textContent=parseFloat(this.value).toFixed(1)+'x';localStorage.setItem('intellect-tts-rate',this.value);};
    }
    const ttsPitchSlider=$('settingsTtsPitch');
    const ttsPitchValue=$('settingsTtsPitchValue');
    if(ttsPitchSlider){
      const savedPitch=localStorage.getItem('intellect-tts-pitch');
      ttsPitchSlider.value=savedPitch||'1';
      if(ttsPitchValue) ttsPitchValue.textContent=parseFloat(ttsPitchSlider.value).toFixed(1);
      ttsPitchSlider.oninput=function(){if(ttsPitchValue)ttsPitchValue.textContent=parseFloat(this.value).toFixed(1);localStorage.setItem('intellect-tts-pitch',this.value);};
    }
    const notifCb=$('settingsNotificationsEnabled');
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'Intellect';
    ...
    const dashboardTitleField=$('settingsDashboardTitle');
    if(dashboardTitleField){
      dashboardTitleField.value=settings.dashboard_title||'Intellect Agent Harness';
      let dashboardTitleTimer=null;
      dashboardTitleField.addEventListener('input',()=>{
        if(dashboardTitleTimer) clearTimeout(dashboardTitleTimer);
        dashboardTitleTimer=setTimeout(_schedulePreferencesAutosave,500);
      },{once:false});
    }
    _syncIntellectPanelSessionActions();
...
async function loadProvidersPanel(){
  ...
  try{
    ...
    oauthSection.className = 'oauth-providers-section';
    oauthSection.innerHTML = '<h3 class="provider-section-title">' + esc(t('providers_oauth_title') || 'OAuth Providers') + '</h3>';
    const oauthContainer = document.createElement('div');
    oauthContainer.className = 'oauth-providers-panel';
    oauthSection.appendChild(oauthContainer);
    list.appendChild(oauthSection);
    if (typeof renderOAuthProvidersPanel === 'function') {
      renderOAuthProvidersPanel(oauthContainer);
    }
  }catch(e){
...
function _providerQuotaPoolShouldDefaultOpen(pool){
  try{
    const saved=localStorage.getItem('intellect-provider-quota-pool-open');
...
function _buildProviderQuotaCard(status){
  ...
  if(poolDetails){
    poolDetails.addEventListener('toggle',()=>{
      try{localStorage.setItem('intellect-provider-quota-pool-open',poolDetails.open?'1':'0');}catch(e){}
    });
  }
  return card;
}
...
function _buildProviderCard(p){
  ...
  // key_source can be 'oauth' (intellect auth), 'config_yaml' (token in config.yaml), or 'none'.
  const isOauth=p.is_oauth===true;
  // models_total reflects the complete catalog size. The "models" array may
  // be trimmed to a featured subset for UI scannability — fall back to its
  // length only when the server didn't supply models_total.
  ...
  if(isOauth){
    ...
    if(p.key_source==='config_yaml'){
      hint.textContent=t('providers_oauth_config_yaml_hint')||'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run intellect auth.';
      ...
      hint.textContent=t('providers_oauth_not_configured_hint')||'Not authenticated. Run intellect auth in the terminal to configure this provider.';
  ...
  if(modelCount>0){
    ...
    // When the rendered list is a strict subset of the total catalog, show
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'Intellect';
  window._dashboardTitle=body.dashboard_title||'Intellect Agent Harness';
  ...
  _settingsFontSizeOnOpen=fontSize||localStorage.getItem('intellect-font-size')||'default';
  const bar=$('settingsUnsavedBar');
  if(bar) bar.style.display='none';
  _settingsIntellectDefaultModelOnOpen=body.default_model||_settingsIntellectDefaultModelOnOpen||'';
...
// Keep in sync with intellect_cli/main.py _AUX_TASKS and api/routes.py _AUX_TASK_SLOTS.
...
async function saveSettings(andClose){
  ...
  const modelChanged=(model||'')!==(_settingsIntellectDefaultModelOnOpen||'');
  ...
  const fontSize=($('settingsFontSize')||{}).value||localStorage.getItem('intellect-font-size')||'default';
...
async function signOut(){
  try{
    if(typeof fetchMembersStatus==='function'){
      ...
      if(status&&status.enabled&&typeof memberSignOut==='function') return memberSignOut();
    }
  }catch(e){/* fall through to legacy sign-out */}
  try{
    if(typeof clearChatUiForSignOut==='function') clearChatUiForSignOut();
    await api('/api/auth/logout',{method:'POST',body:'{}'});
    window.location.replace(new URL('login',document.baseURI||location.href).href);
  }catch(e){
...
async function disableAuth(){
  ...
  try{
    ...
    if(typeof refreshUserProfileChrome==='function') void refreshUserProfileChrome();
  }catch(e){
...
window.addEventListener('intellect:cron_created', () => {
...
switchSettingsSection=function(name){
  ...
  if(name==='system'){loadMcpServers();loadMcpTools();loadGatewayStatus();loadMembersSetupCard();}
};
...
async function loadMembersSetupCard(){
  ...
  try{
    ...
    if(!s||!s.enabled){
      card.innerHTML=`<div style="color:var(--muted);font-size:12px">${esc(t('members_setup_off')||'Members disabled (members.enabled: false).')}</div>`;
      return;
    }
    const origin=(typeof location!=='undefined'&&location.origin)?location.origin.replace(/\/$/,''):'http://127.0.0.1:9119';
    const suggestedBase=origin;
    const suggested=`${origin}/api/members/oauth/callback`;
    const mode=esc(s.mode||'');
    const oauthOn=s.oauth_enabled?esc(t('members_setup_oauth_on')||'OAuth on'):esc(t('members_setup_oauth_off')||'OAuth off');
    const actor=s.actor_member_id?esc(s.actor_member_id):esc(t('member_not_signed_in')||'Not signed in');
    const teamsLine=s.teams_enabled
      ? `<div style="font-size:12px;margin-top:6px">${esc(t('members_setup_teams_on')||'Teams enabled')}</div>`
      : '';
    const oauthHint=s.oauth_enabled?`
      <div class="members-setup-callback" style="margin-top:10px;padding:10px;background:var(--code-bg);border:1px solid var(--border2);border-radius:6px;font-size:12px;line-height:1.5">
        ...
        <div style="color:var(--muted);margin-top:6px">${esc(t('members_setup_callback_base')||'config.yaml base (no path):')} <code>${esc(suggestedBase)}</code></div>
        <div style="color:var(--muted);margin-top:6px">${esc(t('members_setup_callback_hint')||'Set members.oauth.callback_base_url to the base URL above when behind a reverse proxy. Must match Gitee/GitHub app settings exactly (including localhost vs 127.0.0.1 and port).')}</div>
      </div>`:'';
    const localRegLine=`
    ...
    card.innerHTML=`
      <div style="font-size:12px;display:flex;flex-direction:column;gap:4px">
        <span>${esc(t('members_setup_mode')||'Mode')}: <strong>${mode}</strong></span>
        <span>${oauthOn}</span>
        <span>${esc(t('members_setup_actor')||'Signed in as')}: <code>${actor}</code></span>
        ${teamsLine}
      </div>${localRegLine}${oauthHint}`;
  }catch(e){
