...
function _syncWorkspacePanelInlineWidth(){
  ...
  const saved = localStorage.getItem('intellect-panel-w');
  if(!saved) return;
...
function _setWorkspacePanelMode(mode){
  ...
  try{localStorage.setItem('intellect-webui-workspace-panel', open ? 'open' : 'closed');}catch(_){}
...
const _SIDEBAR_COLLAPSED_KEY='intellect-webui-sidebar-collapsed';
...
(function(){
  ...
  function _voiceModePrefEnabled(){
    try{ return localStorage.getItem('intellect-voice-mode-button')==='true'; }
    catch(_){ return false; }
  }
  ...
  function _speakResponse(){
    ...
    const savedVoice=localStorage.getItem('intellect-tts-voice');
    const voices=speechSynthesis.getVoices();
    if(savedVoice&&voices.length){
      const match=voices.find(v=>v.name===savedVoice);
      if(match) utter.voice=match;
    }
    const savedRate=parseFloat(localStorage.getItem('intellect-tts-rate'));
    if(!isNaN(savedRate)) utter.rate=Math.min(2,Math.max(0.5,savedRate));
    const savedPitch=parseFloat(localStorage.getItem('intellect-tts-pitch'));
...
$('btnDownload').onclick=()=>{
  ...
  a.download=`intellect-${S.session.session_id}.md`;a.click();URL.revokeObjectURL(a.href);
};
$('btnExportJSON').onclick=()=>{
  if(!S.session)return;
  const url=`/api/session/export?session_id=${encodeURIComponent(S.session.session_id)}`;
  const a=document.createElement('a');a.href=url;
  a.download=`intellect-${S.session.session_id}.json`;a.click();
};
...
$('modelSelect').onchange=async()=>{
  ...
  else try{localStorage.setItem('intellect-webui-model',modelState.model)}catch{}
  ...
  // Warn if selected model belongs to a different provider than what Intellect is configured for
...
(function(){
  ...
  function initResize(handleId, targetEl, edge, minW, maxW, storageKey){
    ...
    // Restore saved width
    if(storageKey === 'intellect-panel-w'){
      _syncWorkspacePanelInlineWidth();
    }else{
  ...
  window._initResizePanels = function(){
    ...
    initResize('sidebarResize',    sidebar,    'right', SIDEBAR_MIN, SIDEBAR_MAX, 'intellect-sidebar-w');
    initResize('rightpanelResize', rightpanel, 'left',  PANEL_MIN,   PANEL_MAX,   'intellect-panel-w');
  };
})();
...
//   3. Native WKWebView wrappers (e.g. intellect-swift-mac) that read this attribute as
...
function _syncThemeColorMeta(){
  try{
    ...
    if(!bg) return;
    const known=document.getElementById('intellect-theme-color');
    if(known){
...
function _pickTheme(name){
  const currentSkin=localStorage.getItem('intellect-skin');
  const appearance=_normalizeAppearance(name,currentSkin);
  localStorage.setItem('intellect-theme',appearance.theme);
  localStorage.setItem('intellect-skin',appearance.skin);
...
function _pickSkin(name){
  const appearance=_normalizeAppearance(localStorage.getItem('intellect-theme'),name);
  localStorage.setItem('intellect-theme',appearance.theme);
  localStorage.setItem('intellect-skin',appearance.skin);
  _applyTheme(appearance.theme);
  _applySkin(appearance.skin);
...
function _pickFontSize(size){
  localStorage.setItem('intellect-font-size',size);
  _applyFontSize(size);
...
(async()=>{
  ...
  try{
    ...
    const lsTheme=(localStorage.getItem('intellect-theme')||'').trim().toLowerCase();
    const lsSkin=(localStorage.getItem('intellect-skin')||'').trim().toLowerCase();
    ...
    _applyTheme(theme);
    localStorage.setItem('intellect-skin',skin);
    _applySkin(skin);
    ...
    const fontSize=(s.font_size||localStorage.getItem('intellect-font-size')||'default');
    localStorage.setItem('intellect-font-size',fontSize);
    _applyFontSize(fontSize);
    if(typeof setLocale==='function'){
      const _lang=typeof resolvePreferredLocale==='function'
        ? resolvePreferredLocale(s.language, localStorage.getItem('intellect-lang'))
        : (s.language || localStorage.getItem('intellect-lang') || 'en');
      setLocale(_lang);
      if(typeof applyLocaleToDOM==='function')applyLocaleToDOM();
    }
    applyBotName();
    // TTS: apply enabled state on boot so buttons show/hide correctly (#499)
    if(typeof _applyTtsEnabled==='function') _applyTtsEnabled(localStorage.getItem('intellect-tts-enabled')==='true');
  }catch(e){
    ...
    window._botName='Intellect';
    _bootSettings={check_for_updates:false};
    if(typeof setLocale==='function'){
      const _lang=typeof resolvePreferredLocale==='function'
        ? resolvePreferredLocale(null, localStorage.getItem('intellect-lang'))
        : (localStorage.getItem('intellect-lang') || 'en');
      setLocale(_lang);
      if(typeof applyLocaleToDOM==='function')applyLocaleToDOM();
    }
    applyBotName();
  ...
  if(_testUpdates||(_bootSettings.check_for_updates!==false&&!sessionStorage.getItem('intellect-update-checked')&&!sessionStorage.getItem('intellect-update-dismissed'))){
    const _checkUrl='api/updates/check'+(_testUpdates?'?simulate=1':'');
    api(_checkUrl).then(d=>{if(!_testUpdates)sessionStorage.setItem('intellect-update-checked','1');if((d.webui&&d.webui.behind>0)||(d.agent&&d.agent.behind>0))_showUpdateBanner(d);}).catch(()=>{});
  }
  ...
  const _modelDropdownReady=populateModelDropdown().then(()=>{
    const savedState=(typeof _readPersistedModelState==='function')
      ...
      : (localStorage.getItem('intellect-webui-model')?{model:localStorage.getItem('intellect-webui-model'),model_provider:null}:null);
  ...
  if(typeof initMemberAuth==='function') await initMemberAuth();
  await renderSessionList();
  _initResizePanels();
  ...
  const savedLocal=localStorage.getItem('intellect-webui-session');
  ...
  if(saved){
    try{
      ...
      if(S.session && (S.session.message_count||0) === 0 && !_restoredInFlight){
        ...
        const _ephPanelPref=localStorage.getItem('intellect-webui-workspace-panel-pref')==='open'
          || localStorage.getItem('intellect-webui-workspace-panel')==='open';
      ...
      const panelPref=localStorage.getItem('intellect-webui-workspace-panel-pref')==='open'
        || localStorage.getItem('intellect-webui-workspace-panel')==='open';
    ...
    catch(e){localStorage.removeItem('intellect-webui-session');}
  }
  ...
  const _freshPanelPref=localStorage.getItem('intellect-webui-workspace-panel-pref')==='open'
    || localStorage.getItem('intellect-webui-workspace-panel')==='open';
  ...
  console.error('[intellect] boot failed', e);
...
window.addEventListener('pageshow', async (event) => {
  ...
  if (typeof initMemberAuth === 'function') try { void initMemberAuth(); } catch (_) {}
  ...
  if (typeof _isSidebarCollapsed === 'function' && typeof toggleSidebar === 'function') {
    try {
      const _want = localStorage.getItem('intellect-webui-sidebar-collapsed') === '1';
