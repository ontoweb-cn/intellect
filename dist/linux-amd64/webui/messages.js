...
async function send(){
  ...
  try{
    ...
    if(startData.effective_model && S.session){
      ...
      localStorage.setItem('intellect-webui-model', startData.effective_model);
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _streamFadeNextText(targetText){
    ...
    // appear to accelerate. Intellect has a playout buffer, so track incoming word
    ...
    source.addEventListener('cancel',e=>{
      ...
      (async()=>{
        try{
          ...
          if(S.session&&S.session.session_id===activeSid){
            clearLiveToolCards();if(!assistantText)removeThinking();
            const cancelAgentName=((window._botName||'Intellect')+'').trim()||'Intellect';
...
function transcript(){
  const lines=[`# Intellect session ${S.session?.session_id||''}`,``,
...
function _stashClarifyDraft(reason) {
  ...
  const key = `intellect-clarify-draft-${sid}-${_clarifySignature || "unknown"}`;
  try {
...
function sendBrowserNotification(title,body){
  ...
  const botName=window._botName||'Intellect';
