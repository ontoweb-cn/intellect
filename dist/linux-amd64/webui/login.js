/* Login page — single-user (WebUI password) vs multi-user (tabbed member sign-in). */
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderOAuth(providers) {
    ...
    renderMemberOAuthProviders(oauthHost, providers, {
      ...
      onSelect: function (providerId) {
        window.location.href = _memberOAuthUrl(providerId);
      },
      afterRender: function () {
        var ctx = window.__LOGIN_CONTEXT__ || null;
        var uri = ctx && ctx.members ? ctx.members.oauth_callback_uri : '';
        appendOAuthCallbackHint(uri);
      },
    });
  }

  function appendOAuthCallbackHint(callbackUri) {
    if (!oauthHost || !callbackUri) return;
    var existing = oauthHost.querySelector('.oauth-callback-details');
    if (existing) existing.remove();
    var details = document.createElement('details');
    details.className = 'oauth-callback-details';
    var summary = document.createElement('summary');
    summary.textContent = L('oauth_callback_register', 'Register this callback URL in your OAuth app (e.g. Gitee):');
    details.appendChild(summary);
    var codeEl = document.createElement('code');
    codeEl.style.wordBreak = 'break-all';
    codeEl.style.display = 'block';
    codeEl.style.marginTop = '6px';
    codeEl.textContent = callbackUri;
    details.appendChild(codeEl);
    oauthHost.appendChild(details);
  }
  ...
  function applyContext(ctx) {
    ...
    localDevAvailable = Boolean(ctx.show_local_dev_form && _clientIsLocalhost());
    if (tabLocal) tabLocal.classList.toggle('hidden', !localDevAvailable);
    if (localPwInput) {
      localPwInput.hidden = Boolean(ctx.webui_auth_enabled === false);
    }
  ...
  async function loadContext() {
    ...
    var embedded = !loginSignedOut ? window.__LOGIN_CONTEXT__ : null;
