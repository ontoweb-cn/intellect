...
function cliOnlyCommandResponse(cmdName, meta){
  ...
  let extra='';
  if(name==='browser'){
    extra='\n\nBrowser tools in WebUI must be configured server-side with the agent/browser environment. Once configured, ask the model to use browser tools directly; `/browser` itself only works in `intellect chat`.';
  }
  return `\`/${name}\` is a Intellect CLI-only command and cannot run inside the WebUI.${detail}${extra}`;
}
...
async function cmdTheme(args){
  ...
  if(themes.includes(val)||legacyThemes.includes(val)){
    const appearance=_normalizeAppearance(
      val,
      legacyThemes.includes(val)?null:localStorage.getItem('intellect-skin')
    );
  ...
  if(skins.includes(val)){
    const appearance=_normalizeAppearance(localStorage.getItem('intellect-theme'),val);
    localStorage.setItem('intellect-theme',appearance.theme);
    localStorage.setItem('intellect-skin',appearance.skin);
    _applyTheme(appearance.theme);
    _applySkin(appearance.skin);
 ...
 * or doesn't support steer (older intellect-agent versions).
 */
...
function cmdReasoning(args){
  ...
  const BRAIN='\uD83E\uDDE0';
  // Matches intellect_constants.VALID_REASONING_EFFORTS + 'none' (CLI parity).
