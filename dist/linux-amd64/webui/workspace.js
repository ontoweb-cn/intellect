async function api(path,opts={}){
  ...
  const teamId=typeof getWebuiActiveTeamId==='function'?getWebuiActiveTeamId():'';
  if(teamId&&!headers['X-Intellect-Team'])headers['X-Intellect-Team']=teamId;
  ...
  for(let attempt=0;attempt<3;attempt++){
    try{
      ...
      if(!res.ok){
        ...
        // and for subpath mounts like /intellect/, where /login escapes to the site root.
        if(res.status===401){
          if(!/\/login(?:\?|$)/.test(window.location.pathname||'')){
            if(typeof redirectToMemberLogin==='function') redirectToMemberLogin();
            else window.location.href='login?next='+encodeURIComponent(window.location.pathname+window.location.search);
          }
          return;
        }
...
function _wsExpandKey(){
  ...
  return ws?'intellect-webui-expanded:'+ws:null;
}
