...
const SESSION_VIEWED_COUNTS_KEY = 'intellect-session-viewed-counts';
const SESSION_COMPLETION_UNREAD_KEY = 'intellect-session-completion-unread';
const SESSION_OBSERVED_STREAMING_KEY = 'intellect-session-observed-streaming';
...
function _sessionIdVisibleInSidebar(sid){
  if(!sid) return false;
  return (_allSessions||[]).some(s=>s&&s.session_id===sid);
}

/** Wipe in-memory chat state on sign-out (before redirect to /login). */
function clearChatUiForSignOut(){
  try{localStorage.removeItem('intellect-webui-session');}catch(e){}
  if(typeof S!=='undefined'){
    if(typeof cancelStream==='function'&&S.activeStreamId){void cancelStream();}
    S.session=null;
    S.messages=[];
    S.toolCalls=[];
  }
  ...
  const msgInner=typeof $==='function'?$('msgInner'):document.getElementById('msgInner');
  if(msgInner) msgInner.innerHTML='';
  if(typeof syncTopbar==='function') syncTopbar();
  if(typeof renderSessionList==='function') void renderSessionList();
}
if(typeof window!=='undefined') window.clearChatUiForSignOut=clearChatUiForSignOut;

/** Reload sidebar + active chat after member/team context changes (multi-user). */
async function refreshChatAfterMemberContextChange(opts={}){
  const showToastMsg=opts.showToast!==false;
  if(typeof renderSessionList==='function') await renderSessionList();

  let sid=S.session&&S.session.session_id;
  if(!_sessionIdVisibleInSidebar(sid)){
    const first=(_allSessions||[]).find(s=>s&&s.session_id);
    sid=first?first.session_id:null;
  }
  ...
  if(typeof syncTopbar==='function') syncTopbar();
  if(S.session&&typeof loadDir==='function') loadDir('.');

  if(showToastMsg&&typeof showToast==='function'){
    const msg=(typeof t==='function'?(t('teams_context_refreshed')||''):'')||'Active team updated — chat refreshed.';
    showToast(msg);
  }
}
if(typeof window!=='undefined') window.refreshChatAfterMemberContextChange=refreshChatAfterMemberContextChange;
...
async function newSession(flash, options={}){
  ...
  try{localStorage.setItem('intellect-webui-session',S.session.session_id);}catch(_){}
...
async function loadSession(sid, options={}){
  ...
  const prevLoadingSessionId = _loadingSessionId;
  const hasLocalMessages = !!(S.messages && S.messages.length > 0 && S.messages[0] && S.messages[0].role);
  const msgCount = S.session ? Number(S.session.message_count || 0) : 0;
  // Clicking the already-open session is a no-op when its transcript is loaded.
  // Exception: another session's load cleared this pane (A→B then back to A while
  // B is still in flight) — must reload, not return with an empty message array.
  if (currentSid === sid && !force) {
    const abortedSwitchToOther = prevLoadingSessionId && prevLoadingSessionId !== sid;
    const messagesMissing = msgCount > 0 && !hasLocalMessages;
    if (!abortedSwitchToOther && !messagesMissing) return;
  }
  ...
  const mustResetPane = currentSid !== sid || (msgCount > 0 && !hasLocalMessages) || (force && currentSid === sid);
  if (mustResetPane) {
    S.messages = [];
    S.toolCalls = [];
  ...
  try {
    ...
    if(_msgInner){
      if(e.status===404){
        ...
        if(!currentSid&&localStorage.getItem('intellect-webui-session')===sid){
          localStorage.removeItem('intellect-webui-session');
  ...
  if(INFLIGHT[sid]){
    ...
    if(typeof queueSessionMessage==='function'){
      try{
        const _storedQ=sessionStorage.getItem('intellect-queue-'+sid);
        if(_storedQ){
          ...
          if(Array.isArray(_entries)&&_entries.length){
            ...
            if(_fresh.length){
              ...
              sessionStorage.removeItem('intellect-queue-'+sid);
            } else {
              sessionStorage.removeItem('intellect-queue-'+sid);
            }
          } else {
      ...
      }catch(_){sessionStorage.removeItem('intellect-queue-'+sid);}
    }
...
function _setSessionSourceFilter(filter) {
  ...
  try { localStorage.setItem('intellect-session-source-filter', next); } catch (_e) {}
  renderSessionListFromCache();
}

function _restoreSessionSourceFilter() {
  try {
    const raw = localStorage.getItem('intellect-session-source-filter');
...
async function _ensureMessagesLoaded(sid) {
  // Already have messages for this session? (e.g. from INFLIGHT restore path)
  ...
  // Stale? Session switched while the tail-window request was in flight.
  ...
  deleteBtn.onclick=async()=>{
    ...
    try{
      ...
      if(S.session&&ids.includes(S.session.session_id)){
        S.session=null;S.messages=[];S.entries=[];localStorage.removeItem('intellect-webui-session');
...
function _sessionTitleIsDefaultWebUI(rawTitle){
  ...
  return title==='Intellect WebUI'||/^Intellect WebUI #\d+$/.test(title);
}
...
function _activeSessionIdForSidebar(){
  if(_loadingSessionId) return _loadingSessionId;
...
function renderSessionListFromCache(){
  ...
  let _groupCollapsed={};
  try{_groupCollapsed=JSON.parse(localStorage.getItem('intellect-date-groups-collapsed')||'{}');}catch(e){}
  const _saveCollapsed=()=>{try{localStorage.setItem('intellect-date-groups-collapsed',JSON.stringify(_groupCollapsed));}catch(e){}};
...
async function _handleActiveSessionStorageEvent(e){
  if(!e || e.key !== 'intellect-webui-session') return;
...
async function deleteSession(sid){
  ...
  if(S.session&&S.session.session_id===sid){
    ...
    if(remaining.sessions&&remaining.sessions.length){
      ...
      const _tt=$('topbarTitle');if(_tt)_tt.textContent=window._botName||'Intellect';
