...
const MAX_UPLOAD_BYTES=(window.__INTELLECT_CONFIG__&&window.__INTELLECT_CONFIG__.maxUploadBytes)||20*1024*1024;
...
// Handles iOS PWA standalone mode and keeps subpath mounts like /intellect/ from
...
function queueSessionMessage(sid, payload){
  ...
  try{ sessionStorage.setItem('intellect-queue-'+sid, JSON.stringify(q)); }catch(_){}
  return q.length;
}
function shiftQueuedSessionMessage(sid){
  ...
  if(!q.length){
    ...
    try{ sessionStorage.removeItem('intellect-queue-'+sid); }catch(_){}
  } else {
    try{ sessionStorage.setItem('intellect-queue-'+sid, JSON.stringify(q)); }catch(_){}
  }
  return next;
...
const MEDIA_PLAYBACK_STORAGE_KEY='intellect-media-playback-rate';
...
const MODEL_STATE_KEY='intellect-webui-model-state';
...
function _readPersistedModelState(){
  ...
  const legacy=localStorage.getItem('intellect-webui-model');
...
function _writePersistedModelState(model, modelProvider){
  ...
  if(!value){
    localStorage.removeItem('intellect-webui-model');
    localStorage.removeItem(MODEL_STATE_KEY);
    return;
  }
  localStorage.setItem('intellect-webui-model', value);
  try{
...
async function populateModelDropdown(opts={}){
  ...
  try{
    ...
    for(const g of groups){
      ...
      // Hydrate the label map from extra_models too. This keeps a model
      // selected from the slash-command autocomplete or a persisted-localStorage
      // value renderable with its proper label instead of falling back to
      // the bare ID.
    ...
    if(data.default_model && !(typeof _readPersistedModelState==='function'&&_readPersistedModelState()) && !localStorage.getItem('intellect-webui-model')){
...
function _addLiveModelsToSelect(provider, models, sel){
  ...
  // Normalized dedup: strip @provider: prefix and namespace so bare model
  // IDs match prefixed variants from different providers (#907).
  const _normId=id=>{
    let s=String(id||'');
...
async function _fetchLiveModels(provider, sel){
  ...
  try{
    ...
    if(added>0){
      if(typeof syncModelChip==='function') syncModelChip();
      console.debug('[intellect] Live models loaded for',provider+':',added,'new models added');
    }
  }catch(e){
    console.debug('[intellect] Live model fetch failed for',provider,e.message);
  }finally{
 ...
 * currently configured in Intellect. Returns a warning string if mismatched,
...
function _checkProviderMismatch(modelId){
  ...
  if(norm(modelProvider)!==norm(ap)){
    return (window.t?window.t('provider_mismatch_warning',modelId,ap):
      `"${modelId}" may not work with your configured provider (${ap}). Send anyway or run \`intellect model\` to switch.`);
  }
  return null;
}
...
function renderMd(raw){
  ...
  s=s.replace(/\x00D(\d+)\x00/g,(_,i)=>{
    ...
    if(/^https?:\/\//i.test(ref)){
      ...
      // joins cleanly — this preserves any subpath mount (e.g. /intellect/).
      let src=ref;
...
function _renderQueueChips(sid){
  ...
  function _saveAndRefresh(){
    ...
    if(!liveQ.length){delete SESSION_QUEUES[sid];try{sessionStorage.removeItem('intellect-queue-'+sid);}catch(_){}}
    else{SESSION_QUEUES[sid]=[...liveQ];try{sessionStorage.setItem('intellect-queue-'+sid,JSON.stringify(liveQ));}catch(_){}}
  ...
  if(q.length>1){
    ...
    mergeBtn.onclick=()=>{
      const _doMerge=(snapshot)=>{
        ...
        try{sessionStorage.setItem('intellect-queue-'+sid,JSON.stringify(liveQ));}catch(_){}
        delete _queueRenderKeys[sid];
        updateQueueBadge(sid);
      };
      if(hasFiles){
...
function autoReadLastAssistant(){
  ...
  const pref=localStorage.getItem('intellect-tts-auto-read');
  if(pref!=='true') return;
...
const INFLIGHT_KEY = 'intellect-webui-inflight'; // localStorage key for in-flight session tracking
const INFLIGHT_STATE_KEY = 'intellect-webui-inflight-state'; // localStorage snapshots for mid-stream reload recovery
...
// ── Intellect agent/gateway heartbeat alert (#716) ──
...
const WHATS_NEW_SUMMARY_STORAGE_KEY='intellect-whats-new-generated-summaries';
...
function dismissUpdate(){
  ...
  sessionStorage.setItem('intellect-update-dismissed','1');
}
...
async function applyUpdates(){
  ...
  try{
    ...
    sessionStorage.removeItem('intellect-update-checked');
    sessionStorage.removeItem('intellect-update-dismissed');
...
function syncTopbar(){
  if(!S.session){
    document.title=window._botName||'Intellect';
    if(typeof syncWorkspaceDisplays==='function') syncWorkspaceDisplays();
    if(typeof _syncWorkspaceHeadingState==='function') _syncWorkspaceHeadingState();
    if(typeof syncModelChip==='function') syncModelChip();
    if(typeof syncTerminalButton==='function') syncTerminalButton();
    if(typeof _syncIntellectPanelSessionActions==='function') _syncIntellectPanelSessionActions();
    else {
  ...
  document.title=sessionTitle+' \u2014 '+(window._botName||'Intellect');
...
function _assistantRoleHtml(tsTitle='', tpsText=''){
  const _bn=window._botName||'Intellect';
...
const _activityDisclosureStoragePrefix='intellect-activity-disclosure:';
...
function renderMessages(options){
  ...
  if(!S.busy && (!S.toolCalls||!S.toolCalls.length)){
    ...
    S.messages.forEach(m=>{
      if(!m) return;
      // OpenAI / Intellect CLI format: role=tool with tool_call_id
      if(m.role==='tool'){
...
function toggleWorkspaceHiddenFiles(value){
  ...
  try{localStorage.setItem('intellect-workspace-show-hidden-files',S.showHiddenWorkspaceFiles?'1':'0');}catch(_){}
  _syncWorkspaceHiddenToggle();
  renderFileTree();
}
try{S.showHiddenWorkspaceFiles=localStorage.getItem('intellect-workspace-show-hidden-files')==='1';}catch(_){}
